...
void setup()
{
  ...
  if (EEPROM.read(550) == 0 || EEPROM.read(550) == 255) {// komórka kontrolna - jesli inny reset & save
    ...
    oled.clear();
    oled.print("ArtGateOne LED 1.1");
    delay(1500);
  }
  else {//dhcp
    oled.clear();
    oled.println("ArtGateOne LED 1.1");
    oled.print  ("DHCP...");
    delay(1500);
    Ethernet.begin(mac);
    Ethernet.maintain();
...
void loop()
{
  ...
  if (packetSize == 14) {
    ...
    buf[10] = Ethernet.localIP()[0]; // IPV4 [0]
    buf[11] = Ethernet.localIP()[1]; // IPV4 [1]
    buf[12] = Ethernet.localIP()[2]; // IPV4 [2]
    buf[13] = Ethernet.localIP()[3]; // IPV4 [3]

    buf[14] = 0x36; // IP Port Low
    buf[15] = 0x19; // IP Port Hi

    buf[16] = 0x01; // High byte of Version
    buf[17] = 0x01; // Low byte of Version

    buf[18] = intN; // NetSwitch
    buf[19] = intS; // Net Sub Switch
    buf[20] = 0xFF; // 0x04; // OEMHi
    buf[21] = 0xFF; // 0xB9; // OEMLow
    buf[22] = 0x00; // Ubea Version
    buf[23] = 0xF0; // Status1
    buf[24] = 0x00; // ESTA LO 0x41; //
    buf[25] = 0x00; // ESTA HI  0x4D; //

    buf[26] = byte('A');  // A  //Short Name
    buf[27] = byte('r');  // r
    buf[28] = byte('t');  // t
    buf[29] = byte('G');  // G
    buf[30] = byte('a');  // a
    ...
    buf[172] = 0x00; // NumPorts Hi
    buf[173] = 0x01; // NumPorts Lo
    buf[174] = 0x80; // Port 0 Type
    buf[175] = 0x00; // Port 1 Type
    buf[176] = 0x00; // Port 2 Type
    buf[177] = 0x00; // Port 3 Type
    buf[178] = 0x00; // GoodInput 0
    buf[179] = 0x00; // GoodInput 1
    buf[180] = 0x00; // GoodInput 2
    buf[181] = 0x00; // GoodInput 3
    buf[182] = 0x80; // GoodOutput 0
    buf[183] = 0x00; // GoodOutput 1
    buf[184] = 0x00; // GoodOutput 2
    buf[185] = 0x00; // GoodOutput 3
    buf[186] = 0x00; // SwIn 0
    buf[187] = 0x00; // SwIn 1
    buf[188] = 0x00; // SwIn 2
    buf[189] = 0x00; // SwIn 3
    buf[190] = intU; // SwOut 0//ODBIERA UNIVERSE NR
    buf[191] = 0x00; // SwOut 1
    buf[192] = 0x00; // SwOut 2
    buf[193] = 0x00; // SwOut 3
    buf[194] = 0x01; // SwVideo
    buf[195] = 0x00; // SwMacro
    buf[196] = 0x00; // SwRemote
    buf[197] = 0x00; // Spare
    buf[198] = 0x00; // Spare
    buf[199] = 0x00; // Spare
    buf[200] = 0x00; // Style
    // MAC ADDRESS
    buf[201] = mac[0]; // MAC HI
    buf[202] = mac[1]; // MAC
    buf[203] = mac[2]; // MAC
    buf[204] = mac[3]; // MAC
    buf[205] = mac[4]; // MAC
    buf[206] = mac[5]; // MAC LO

    buf[207] = 0x00; // BIND IP 0
    buf[208] = 0x00; // BIND IP 1
    buf[209] = 0x00; // BIND IP 2
    buf[210] = 0x00; // BIND IP 3
    buf[211] = 0x00; // BInd Index

    buf[212] = 0x05; // Status2
    // 212 + 26 = 238

    for (int i = 213; i <= 239; i++) {  //Filler
      buf[i] = 0x00;
    }
    Udp.write(buf, 239);    //  bufLen = 239;
    Udp.endPacket();
  }
  if (packetSize == 530) {
    ...
    if (packetBuffer[15] == intN && packetBuffer[14] == intUniverse) { //check artnet universe

      int ch = 0;
      for (int i = 0; i <= strip.numPixels(); i = i + 3) {
        Udp.read(packetBuffer, 3);
        strip.setPixelColor(ch, packetBuffer[0], packetBuffer[1], packetBuffer[2]);
        ch++;
      }
      Udp.read(packetBuffer, 2);
      //strip.setBrightness(packetBuffer[1]);//Master Dimmer
      strip.show();
    }
  }
}//end loop()
